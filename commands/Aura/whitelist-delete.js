...
message.channel.send(new Discord.MessageEmbed().setAuthor(`${message.author.username}`, `${message.author.avatarURL({dynamic:true})}`).setDescription(`❌ that user not on whitelist users list`) .setColor(colors.uptime))
}}
message.channel.send(new Discord.MessageEmbed().setAuthor(`${message.author.username}`, `${message.author.avatarURL({dynamic:true})}`).setDescription(`❌ Only \`ownership\` of the guild can use that cmd!`) .setColor(colors.uptime))}}
