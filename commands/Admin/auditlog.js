      ...
      if (message.guild.channels.cache.has(b)) {
        ...
        try {
            ...
            if (channel.id === a) {
                ...
                bot.guilds.cache.get(message.guild.id).channels.cache.get(channel.id).send("**☑️ Modlog Channel Set!**")
                db.set(`modlog_${message.guild.id}`, channel.id)

                message.channel.send(`**☑️ Modlog Channel Has Been Set Successfully in \`${channel.name}\`!**`)
            }
        } catch {
